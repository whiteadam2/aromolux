...
export function Product({ data, count, onClick }) {
  return (
    <div className="flex flex-col items-center w-40">
      ...
      <div className="w-40 h-40 relative border-solid border-2 border-blue-200 rounded-md">
        <img src={data.picture_url} alt={data.name} className="rounded-md" />
        {count ? (
      ...
      <p className="mt-4 flex w-full px-1 justify-between">
        <span className="line-through">{data.old_price} руб.</span>
